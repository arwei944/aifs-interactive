...
这就是整个想法。变形金刚扩展了它。自我关注将其应用于单个序列。多头注意力并行运行。但2014年的版本已经打破了瓶颈，一旦你有了它，变压器的关键是工程，而不是概念。
...
这是一个变压器注意力层。查询批5个位置，键/值批10个位置，各128个点，8头。`output` is the new context-augmented queries. `weights` is the 5x10 alignment matrix you can visualize.
