...
这似乎是一个小细节。它不是。在几乎所有任务上，AdamW收敛到比Adam + L2正则化更好的解决方案。它是PyTorch中用于训练Transformer、扩散模型和大多数现代架构的默认优化器。BERT， GPT, LLaMA，稳定扩散-都与AdamW训练。
...
类型——> |“Transformer/ LLM”| AdamW[“AdamW < br / > lr = 1的军医,wd = 0.01 - -0.1)
