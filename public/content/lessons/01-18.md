...
如果函数f的定义域是凸集，且对于定义域内任意两点x， y和[0,1]中的任意一点t，函数f是凸的：
...
**测试3：定义测试。**直接检查不等式f(tx + (1-t)y) <= t*f(x) + (1-t)*f(y)。对于导数难以计算的函数很有用。
...
约束||w||^2 <= t定义了一个球（2D的圆，3D的球）。解决方案是损失轮廓首先接触这个球的地方。
