...
> U-Net不是扩散的秘密。将其替换为变压器，将噪声时间表替换为直线流，突然之间您就拥有了SD3， FLUX和每个2026文本到图像模型。
...
- 从U-Net DDPM（第10课）到扩散变压器（DiT）， MMDiT （SD3）和单+双流DiT （FLUX）的演变
...
每一个2026年最先进的文本到图像模型都已经超越了它。稳定扩散3，FLUX, SD4, Z-Image, Qwen-Image，浑源- image -都不使用U-Net。他们使用扩散变压器（DiT）。SD3和FLUX还将DDPM噪声时间表交换为整流，这简化了从噪声到数据的路径，并实现了一致性或蒸馏变量的1-4步推理。
