...
这对ML很重要，因为频域思维无处不在。卷积神经网络执行卷积，也就是在频域中的乘法。Transformer位置编码使用频率分解来表示位置。音频模型（语音识别、音乐生成）基于声谱图——声音的频率表示。时间序列模型寻找周期性模式。理解傅里叶变换给了你处理所有这些的词汇。
...
上面def 的DFT是O(N^2)：对于N个输出系数中的每一个，对N个输入样本求和。对于N = 100万，就是10^12次运算。
...
- [Lee-Thorp等人：FNet：混合令牌与傅里叶变换（2021）](https://arxiv.org/abs/2105.03824) -用FFT替换Transformer中的自关注
