...
- 从头开始实现PCA：中心数据，计算协方差矩阵，特征分解和项目
- 采用解释方差比和肘形法选择主成分个数
- 比较PCA、t-SNE和UMAP在2D中可视化MNIST数字的方法，并解释它们的利弊
- 将核PCA与RBF核结合，分离出标准PCA无法处理的非线性数据结构

## 这个问题

您有一个数据集，每个样本有784个特征。也许是手写数字的像素值。也许是基因表达水平的问题。也许是用户行为信号。你无法想象784个维度。你不能把它们画出来。你甚至不能去想它们。

但这784项功能中的大多数都是多余的。实际的信息存在于一个更小的表面上。一个手写的“7”不需要784个独立的数字来描述它。它需要一些：笔划的角度，横梁的长度，它倾斜的程度。其余的都是噪音。

降维发现，较小的表面。它将784维的数据压缩为2、10或50维，同时保持重要的结构。

## 这个概念

### 维度的诅咒

高维空间是不直观的。随着维度的增长，有三件事被打破了。

**距离变得毫无意义。**在高维情况下，任意两个随机点之间的距离收敛于同一值。如果每个点与其他点的距离大致相同，则最近邻搜索停止工作。
...
**音量集中在角落。** d维的单位超立方体有2^d个角。在100个维度中，几乎所有的体积都在远离中心的角落里。数据点扩散到边缘，你的模型在内部缺乏数据。

**你需要指数级增长的数据。**为了在一个空间中保持相同的样本密度，从2D到20D意味着你需要10^18倍的数据。你永远不会满足。减少维数使数据密度回到可行的水平。

### PCA：找到重要的方向

主成分分析（PCA）找到数据变化最大的轴。它会旋转你的坐标系统，所以第一个轴捕获最大的方差，第二个轴捕获次大的方差，以此类推。
...
为什么eigendecomposition ?协方差矩阵是对称半正定的。它的特征向量在特征空间中是正交的。特征值告诉你每个方向捕获了多少方差。特征值最大的特征向量指向方差最大的方向。

“‘mermaid
图LR
A[“原始数据（2D）\nData在\nx和y方向上的点差”]->|“PCA旋转”| B[“在PCA\nPC1捕获长点差后\nPC2捕获窄点差\nDrop PC2，你丢失的信息很少”]
```
...
T = np.random。均匀的(0,2 * np。π,n_samples)
X1 = 3 * np。Cos (t) + np，随机的。正态（0,0.2,n_samples）
X2 = 3 * np。Sin (t) + np随机。正态（0,0.2,n_samples）
X3 = 0.5 * x1 + 0.3 * x2 + np.random。Normal （0,0.1, n_samples）

X_synthetic = np。Column_stack （[x1, x2, x3]）

pca = pca （n_components=2）
x_reduce = pca.fit_transform（X_synthetic）

打印（f“原始形状：{X_synthetic.shape}”）
print（f" reduce shape: {X_reduced.shape}"）
print（f“解释的方差比率：{pca.explained_variance_ratio_}”）
...
print（f"\nOur PCA解释方差：{pca_2d.explained_variance_ratio_}"）
print（f"Sklearn PCA解释方差：{sklearn_pca.explained_variance_ratio_}"）

Diff = np.abs(np；- np.abs（X_sklearn_pca）
...
结果= {}
对于[10,30,50,100,200]中的k：
pca_k = SklearnPCA（n_components=k）
X_tr = pca_k.fit_transform（X_train）
pca_k.transform（X_test）

clf = LogisticRegression（max_iter=1000, random_state=42）
clf.fit (X_tr y_train)
acc = accuracy_score（y_test, clf.predict(X_te)）
Var_captured = sum（pca_k.explained_variance_ratio_）
结果[k] = （acc, var_capture）
打印(f”k ={凯西:> 3 d} = {acc:准确性。4 f方差}= {var_captured: .4f}”)
```
...
|------|----------------|----------------------|
| Curse of dimensionality | "Too many features" | Distances, volumes, and data density all behave  as dimensions grow. Models need exponentially more data to compensate. |
