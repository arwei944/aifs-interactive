...
- 使用
- 在小型自def 数据集上微调使用LoRA的稳定扩散，并在推理时加载LoRA适配器
...
`num_inference_steps=25` with the default DPM-Solver++ matches `num_inference_steps=50` with DDIM.

### 步骤2：交换调度程序

”“python
从扩散器导入DPMSolverMultistepScheduler， 

管道。scheduler = DPMSolverMultistepScheduler.from_config（pipe.scheduler.config）
管道。scheduler =  .from_config（pipe.scheduler.config）
```
