...
> 每次会议开始时都要交税。代理读取相同的文件，重新尝试相同的探测，并重新发现相同的路径。初始化脚本支付一次税，并将答案写入状态。
...
- 确定代理在每个会话中不应该重做的工作。
- 构建一个确定性初始化脚本，用于探测运行时、依赖项和回购运行状况。
- 持久化探测结果，以便代理读取它，而不是重新运行检查。
- 大声、快速地失败，并且在初始化失败时只有一个地方可以查看。

## 这个问题

打开会话。代理猜测Python版本。猜测test命令。列出五次回购根以找到入口点。尝试导入未安装的包。询问用户配置文件的位置。当它进行真正的编辑时，已经有一万个令牌用于设置工作，而这本来应该是一个脚本。

修复方法是在代理执行任何其他操作之前运行一个初始化脚本，并写入一个
...
Init——> Probes[探测runtime / deps / paths / env / tests]
probe—> Report[init_report.json]
报告——>决策{运行正常？｝
决策—yes—> Agent[Agent Loop]
决定- no -> Halt[大声失败，停止，向人显现]
```
