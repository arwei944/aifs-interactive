...
> 编写代码的代理无法对其进行分级。审阅者是第二个循环，具有不同的系统提示、不同的目标，并且对构建器生成的所有内容具有只读访问权限。构建者和审查者之间的差距是最可靠的地方。
...
- 说明为什么同一代理不能可靠地审查自己的工作。
- 构建一个审阅者代理循环，它使用构建器构件并发出结构化的审阅报告。
- 编写一个对特定维度（而不是共鸣）进行分级的评审标准。
- 将审阅者连接到工作台中，这样人工审阅步骤就可以从真正的工件开始。

## 这个问题

你要求代理修复一个bug。它编辑四个文件，运行测试，并完成报告。验证门（阶段14.38）确认验收运行和范围保持。门上写着你合并。两天后，您发现修复解决了错误的一半。

接受是必要的，而不是充分的。审稿人问了验收不能问的问题：这样解决了正确的问题吗？它是否在没有标记的情况下扩大了范围？它是否记录了应该受到质疑的假设？它是否使工作台处于下一个会话可以拾取的状态？

## 这个概念

“‘美人鱼
流程图LR
建造者[建造者代理]—> Artifacts[diff + state + feedback + verdict]
工件——>审稿人[审稿人代理]
查看者—>标题[reviewer_checklist.md]
