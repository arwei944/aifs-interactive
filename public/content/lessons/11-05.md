...
**系统提示符**：设置角色、约束和行为规则。这是第一个，在转弯时保持不变。Claude Code使用大约6000个令牌作为其系统提示符，包括工具def 和行为指令。保持紧绷。系统提示符中的每个单词在每个API调用中都会重复。
...
**工具修剪**：分类用户的查询意图，只包括与该意图相关的工具。代码问题不需要日历工具。调度问题不需要文件系统工具。这可以将工具def 从8,000个令牌减少到1,000个。
