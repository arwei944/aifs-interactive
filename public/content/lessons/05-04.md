...
> Word2Vec每个单词训练一个嵌入。GloVe对共生矩阵进行因式分解。FastText嵌入了这些碎片。BPE桥接变压器。
...
第三，一旦变形金刚到来，问题又转移了。单词级别的词汇最多可达100万个；真正的语言比这更开放。**字节对编码(BPE)**及其相关技术通过学习涵盖所有内容的频繁子词单位的词汇表来解决这个问题。每个现代LLM的每个现代标记器都是子词标记器。
...
当用户对预训练的LLM进行微调时，拒绝推荐训练自定义标记器。拒绝为任何以生产推理为目标的模型推荐词级标记化。将非英语/多脚本语料库标记为需要带字节回退的sentencepece。
```
