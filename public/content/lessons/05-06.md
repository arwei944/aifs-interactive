...
本课将经典路径（基于规则的，HMM， CRF）引入现代路径（BiLSTM-CRF，然后是变压器）。每一步都解决了前一步的一个特定限制。模式就是教训。
...
- 比起* *的基于变压器。**带有标记分类头的微调BERT。最好的准确性。大多数计算。
...
1. 的方法。基于规则的+地名词典，CRF, BiLSTM-CRF，或变压器微调。
2. 模型开始。命名它（空间模型ID，拥抱脸检查点ID，或“自定义，从头开始训练”）。
3. 标记策略。BIO， BILOU或基于span的。用一句话证明。
4. 评估。使用始终报告实体级F1（而不是令牌级）。

除非用户已经有一个预训练的领域模型，否则拒绝推荐对低于500个标记示例的变压器进行微调。将嵌套实体标记为需要基于跨度或多通道模型。如果用户提到“生产规模”和标签与CoNLL-2003相同，则要求对地名表进行审核。
```
