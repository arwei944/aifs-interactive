...
“亚当的王国”专栏才是真正的罪魁祸首。Adam存储每个参数的运行均值(m)和运行方差(v)，这两个参数都是FP32。对于型号70B，即70B x 4字节x 2 = 560GB。仅优化器就需要7个a100。
...
DeepSpeed的ZeRO（零冗余优化器）在概念上与FSDP相同，但它是由微软独立开发的。它定义了三个阶段，每个阶段都更加分散：
