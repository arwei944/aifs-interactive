...
2. **稀疏/阻塞注意力。**仅选对其余的则被迫减重为零。Longformer, BigBird， OpenAI稀疏Transformer。
3. **微分的注意。**计算两个具有独立Q/K投影的注意图，从另一个中减去一个。消除“注意力下沉”，这种下沉会让前几个代币失去分量。微软的DIFFTransformer（2024）。
...
**质量成本。**仅swaTransformer与远程检索斗争。解决方法：将SWA层与全注意力层交叉。Gemma 3使用5:1 SWA:global。Mistral 7B使用因果- swa堆栈，其中信息通过重叠的窗口“向前流动”-每层扩展有效的接收场

### 稀疏/块注意

选一个三个典型的形状：

- **本地+跨步（OpenAI稀疏Transformer）。**注意最后一个捕捉本地和远程信号
- **Longformer / BigBird。**本地窗口+一小组全局令牌(例如：经验2x上下文匹配质量。
- **原生稀疏注意（DeepSeek, 2025）。**学习哪块跳过内核级别的零块。FlashAttention-compatible。

稀疏注意力是一个核工程的故事。数学很简单（掩盖分数矩阵）；胜利来自于从不将零条目加载到SRAM中。FlashAttention-3和2026 FlexAttention API使自def 稀疏模式在PyTorch中成为一级。

### 差分注意（DIFFTransformer，2024）
...
这将编译成自def 的Triton内核。在常见模式的FlashAttention-3速度的10%以内，并且mask函数是Python可调用的。

**何时挑选：**

- **纯粹的充分关注** -每层高达~16K上下文，或当检索质量是至关重要的。
- **SWA +全局混合** -长上下文（>32K），训练和推理内存约束。2026年默认32K以上。
- **稀疏块注意** -自def 内核，自def 模式。为专门的工作负载（检索、音频）保留。
