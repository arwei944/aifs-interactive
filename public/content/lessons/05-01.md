...
**标记化**将字符串分割成标记。“令牌”故意含糊，因为正确的粒度取决于任务。经典自然语言处理的词水平。变压器的子词。没有空格的语言的字符。
...
Z0`PorterStemmer` runs all five phases. `WordNetLemmatizer` needs the POS tag translated from NLTK's Penn Treebank scheme to WordNet's abbreviation set. 上面的翻译连接是大多数教程跳过的部分。
