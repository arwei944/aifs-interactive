...
- 说出三个评估层的名称——静态基准测试、自def 离线、在线生产——以及每个评估层的用途。
- 解释一下求值器-优化器的紧密循环。
...
2. **自def 离线评估** -您的产品的形状：
   - LLM，法官（凤凰城Langfuse， Opik - Lesson 24）。
   - 基于执行（运行补丁，检查测试）。
...
- **LLM无接地判断。**法官也会产生幻觉。批评家模式（第05课）-根据外部工具进行判断。
...
2. 为你的领域建立一个LLM评判标准，包括三个维度（事实、语气、范围）。完成50个回合。
