...
> 视频是一系列图像加上连接它们的物理原理。每个视频模型都将时间视为一个额外的轴（3D conv），一个需要关注的序列（Transformer），或者一个提取一次和池的特征（2D+池）。
...
一个30秒的视频，每秒30帧，有900张图像。简单地说，视频分类就是图像分类，运行900次，然后进行某种聚合。当动作几乎在每一帧（运动、烹饪、锻炼视频）中都可见时，这种方法就有效；当动作由运动本身def 时，这种方法就严重失败了：“从左向右推某物”在每一帧中看起来就像两个静止的物体。
...
V—> A3[“时空<br/>Transformer<br/>注意<br/>(t, h, w)令牌”]
