...
3. **标准编码器-解码器Transformer。**编码器消耗对数谱图。解码器自动回归生成文本标记。没有声码器，没有CTC，没有HMM。
...
一个24层（大型）Transformer编码器超过1500个时间步长。正弦位置编码，自注意，GELU FFN。产生1500 × 1280个隐藏状态。

### 步骤4 -解码器

24层Transformer解码器。它自回归地从BPE词汇表生成标记，该词汇表是GPT-2的超集，具有一些特定于音频的特殊标记。
