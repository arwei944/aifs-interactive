...
今天生产中的每个LLM应用程序- Perplexity， Cursor, ChatGPT， Notion AI -都解决了这些问题。而不是通过更聪明地处理提示。通过对工程的严格要求。
...
每个严肃的LLM应用程序都遵循相同的流程。具体情况各不相同。结构却没有。
...
**跟踪。**单个用户请求涉及5-8个组件。OpenTelemetry追踪让你看到整个过程：嵌入花了多长时间？是缓存命中吗？LLM的电话打了多久？护栏是否增加了延迟？没有跟踪，调试生产问题就是猜测。
