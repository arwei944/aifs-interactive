...
抽取QA在2026年是不流行的，因为拥有法学硕士的RAG处理更多的案例。它仍然在需要字面引用的环境中发布：法律研究，法规遵从性，审计工具。
...
1. 体系结构。提取，带提取阅读器的RAG，带生成阅读器的RAG，或者闭卷法学硕士。一句话的原因。
2. 寻回犬。无，BM25，密集（命名编码器），或混合。
3. 读者。小组调优模型，名称为LLM，或“领域调优蒸馏酒”。
4. 评估。EM + F1为采掘基准；回答准确性+引用准确性+拒绝校准生产。说出你在测量什么以及你是如何测量它的。

对于监管或合规敏感问题，拒绝闭卷式法学硕士答案。拒绝任何没有检索-召回基线的QA系统（在不知道检索器找到了正确的段落之前，您无法评估阅读器）。将需要多跳推理的问题标记为需要专门的多跳检索器（如hotpotqa训练的系统）。
```
