...
> 一个有能力的模型是不够的。可靠的代理需要一个工作台：指令、状态、范围、反馈、验证、审查和移交。除去这些因素，即使是前沿模式也会产生不安全的产品。
...
- 将模型能力与执行可靠性分开。
- 命名决定代理是否发布的七个工作台表面。
- 在小型回购任务上比较仅使用提示符的运行与使用工作台指导的运行。
- 生成故障模式报告，将每个丢失的表面映射到它所引起的症状。

## 这个问题

您将前沿模型放入实际的repo中，并要求它添加输入验证。它打开四个文件，编写可信的代码，声明成功，然后停止。你来做测试两个失败。第三个文件与验证无关。没有记录表明代理假设了什么，首先尝试了什么，或者还剩下什么要做。

这个模型对Python并没有错。关于工作的问题是错误的。它不知道什么算完成了，允许在哪里编写，哪些测试是权威的，或者下一个会话应该如何开始。

这不是一个模型错误。这是一个工作台错误。代理人周围的表面失去了将一次性一代转变为可靠的，可恢复的工程的部分。

## 这个概念

工作台是在任务期间包装模型的操作环境。它有七个表面：
...
工作台独立于模型。你可以交换模型，保留表面。你不可能在交换表面的同时保持可靠性。
