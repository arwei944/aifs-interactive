...
** 1。独立Q-learning /独立PPO （IQL， IPPO）。**每个代理学习自己的Q或策略，将其他代理视为环境的一部分。很简单，有时它是有效的（特别是当经验回放作为平滑代理建模技巧时）。理论收敛：没有。在实践中：对于松耦合的任务很好，对于紧耦合的任务就不好了。
...
**沟通。**允许座席发送学习到的消息在合作环境下工作。Foerster等人（2016）表明，可微分的代理间通信可以端到端进行训练。今天基于法学硕士的多代理系统（第16阶段）基本上用自然语言进行通信。
