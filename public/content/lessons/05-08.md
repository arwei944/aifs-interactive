...
在Transformer到来之前，两个家族的建筑填补了这一空白。
...
- **边缘/设备上推理。**带有手套嵌入的TextCNN比Transformer小10-100倍。如果您的部署目标是手机，那么这就是堆栈。
- **流/在线分类。** RNN每次处理一个令牌；Transformer需要完整的序列。对于实时输入文本，lstm仍然占上风。
...
1. 编码器架构：TextCNN， BiLSTM, BiLSTM- crf，Transformer微调，或“使用预训练的Transformer作为冻结编码器+小头”。
2. 嵌入输入：随机初始化，GloVe / fastText冻结，或情境化的Transformer嵌入。
3. 5行训练配方：优化器，学习率，批大小，epoch，正则化。
4. 一个监控信号。对于RNN/CNN模型：注意力机制缺失意味着它们错过了远距离深度；检查每长度精度。对于Transformer：如果LR过高，微调崩溃；检查列车损失。

当数据低于~500个标记示例而没有显示TextCNN / BiLSTM基线已经趋于稳定时，拒绝推荐微调Transformer。将边缘部署标记为首先需要架构。
```
