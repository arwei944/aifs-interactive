...
> 精确匹配和F1错过了语义等价。人工审核无法规模化。LLM作为法官是生产答案——有足够的校准来相信这个数字。
...
- ** DeepEval。**LLMPytest。g评估，任务完成，幻觉，偏差指标。CI / CD-native。
- ** G-Eval。**方法（和DeepEval指标）：LLM作为法官，具有思维链，自def 标准，0-1分。

这三家公司都依赖LLM作为法官。本课为方法及其周围的信任层建立了直观的认识。
...
为什么有效：LLM近似于人类的判断，而成本只是人类的一小部分。gpt - 40 -mini每个得分案例约0.003美元，可以以低于5美元的价格运行1000个样本的回归评估。
...
** G-Eval。**def 一个自定义标准：“答案是否引用了正确的来源？”该框架会自动展开为思维链评估步骤，然后打分0-1。对于RAGAS没有涵盖的领域特定的质量维度来说是好的。
...
1. 指标。忠实度/相关性/上下文精确度/上下文召回率+任何自def 的G-Eval标准。
