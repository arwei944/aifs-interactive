...
> 图像是由小块组成的网格。句子是由符号组成的网格。同一个变压器把两者都吃掉了。
...
在2020年之前，计算机视觉意味着卷积。ImageNet、COCO和检测基准上的每个SOTA都使用CNN骨干网。变形金刚是用来表达语言的。

Dosovitskiy等人(2020)-“一张图像值16x16个单词”-表明你可以完全放弃卷积。将图像分割成固定大小的小块，将每个小块线性投影到嵌入中，将序列馈送到普通的变压器编码器。在足够的规模下（ImageNet-21k预训练或更大），ViT可以匹配或击败基于resnet的模型。

ViT是2026年更广泛模式的开始：一种架构，多种模式。Whisper对音频进行标记。ViT标记图像。机器人的动作令牌。视频的像素令牌。变压器不在乎——给它一个序列，它就会学习。
