...
在2015年，TensorFlow要求你在运行任何东西之前def 一个静态计算图。你建立了图表，编译了它，然后通过它输入数据。调试意味着盯着图形可视化。改变架构意味着从头开始重建图。
...
4. **构建自def 数据集。**下载Fashion-MNIST（与MNIST格式相同，但有服装项目）。实现a训练相同的MLP并比较准确性。Fashion-MNIST更难——预计88% vs 98%。
