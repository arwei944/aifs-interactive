...
> 法学硕士什么都做不了。它们生成文本。这就是整个能力。它们不能查看天气、查询数据库、发送电子邮件、运行代码或读取文件。你所见过的每一个“AI代理”都是一个生成JSON的LLM，它会告诉你要调用哪个函数——然后你的代码就会调用它。模型就是大脑。工具是手。功能调用是连接它们的神经系统。
...
MCP是Anthropic的工具互操作性开放标准。与每个应用程序定义自己的工具不同，MCP提供了一个通用协议：工具由MCP服务器提供服务，由MCP客户端（如Claude Code、Cursor或您的应用程序）使用。

一个MCP服务器可以向任何兼容的客户端公开工具。Postgres MCP服务器提供任何MCP兼容的代理数据库访问。GitHub MCP服务器提供任何代理库访问权限。工具定义一次，就可以在任何地方使用。
...
构建一个存储工具定义及其实现的注册表。每个工具都有一个JSON Schema定义（模型看到的内容）和一个Python函数（代码执行的内容）。
