...
抽象摘要是一个生成问题。Transformer根据输入产生新的文本。输出是流畅和压缩的，但可能会产生源中没有的幻觉。风险在于自信的捏造。
...
**摘要式的。**在文档-摘要对上微调Transformer编码器-解码器（BART, T5, Pegasus）。在推理时，模型读取文档并通过交叉注意逐个标记生成摘要。Pegasus特别使用了一个间隙句子预训练目标，使其在没有太多微调的情况下出色地进行总结。
...
3. **难。**将BART-large-CNN与LLM（Claude或GPT-4）在50篇CNN/DailyMail文章中进行比较。报告ROUGE-L，事实（按实体F1）和每个摘要的成本。记录每个成功的地方。
