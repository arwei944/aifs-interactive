...
> 把图像切成小块，把每个小块当作一个字，运行一个标准的Transformer。不要回头看。
...
- 从头开始实现补丁嵌入、学习位置嵌入、类标记和Transformer编码器块，以构建最小的ViT
...
十年来，卷积一直是计算机视觉的代名词。cnn有很强的归纳偏见——局部性、翻译等价性——没有人认为你可以取代。然后Dosovitskiy等人（2020）表明，在没有卷积机器的情况下，将一个普通的Transformer应用于平坦的图像补丁，可以在规模上匹配或击败最好的cnn。

捕获是“大规模的”。ViT在ImageNet-1k上输给了ResNet。ViT在ImageNet-21k或JFT-300M上进行预训练，然后在ImageNet-1k上进行微调，击败它。结论是，Transformer缺乏有用的先验，但可以从足够的数据中学习它们。随后的工作（DeiT， MAE， DINO）表明，通过正确的训练配方——强增强、自监督预训练、蒸馏——ViTs在小数据上也能很好地训练。

到2026年，纯cnn在边缘设备上仍然具有竞争力（ConvNeXt是最强的），但Transformer在其他方面占据主导地位：分割（Mask2Former, SegFormer），检测（DETR, RT-DETR），多模态（CLIP, SigLIP），视频（VideoMAE， VJEPA）。我们需要了解ViT块结构。
