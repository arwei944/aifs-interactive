...
> 单词标记器会被看不见的单词卡住。字符标记器增加了序列长度。子词标记器消除了差异。每个现代法学硕士都有一个。
...
2026年的每个前沿法学硕士都使用三种算法（BPE, Unigram, WordPiece）中的一种，封装在三个库（tiktoken, sentencepece, HF Tokenizers）中的一种。如果没有选择一个语言模型，就不能发布一个语言模型。
...
sp = spm. sentencepeceprocessor （model_file="my_tokenizer.model"）
