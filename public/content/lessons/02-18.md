...
- 从头开始实现过滤方法（方差阈值、互信息、卡方）和包装方法（RFE、前向选择）
- 解释为什么互信息捕捉非线性特征-目标关系，而相关性错过了
- 比较L1正则化（嵌入式选择）和RFE（包装器选择），并评估它们的计算权衡
- 构建一个结合多种方法的特征选择管道，并演示在保留数据上改进的泛化

## 问题是

你有500个功能。你的模型训练缓慢，不断过拟合，没有人能解释它学到了什么。您添加了更多的功能，希望提高性能。更糟的是。

这是在行动的维度的诅咒。随着特征数量的增加，特征空间的体积也会爆炸。数据点变得稀疏。点之间的距离收敛。该模型需要指数级增长的数据来找到真正的模式。噪声特征掩盖了信号特征。过度拟合成为默认值。

ywdbavjje9dqs8ezmjar

xo9wipolpubovdyzpybv

## 这个概念

### 特征选择的三种类型

t5jm5ikcbifeuag3dyx1
...
### dh4mstb0avxd7h5kg3yj

RFE是一个包装器方法。它使用模型自身的特征重要性进行迭代修剪：

1. 用所有特征训练模型
2. bk2wpxqtporgy5v7p5mn
3. 删除最不重要的功能
4. rfz0mv85d8ip0imlaxxj

“‘美人鱼
流程图TD
A[“Start: All N Features”]—b> B[“Train Model”]
B—> C[“Rank Feature importance”]
C—> D[“删除最不重要的”]
D -> E{“功能==目标计数？”｝
E——>|不| B
E—>|是| F[“返回选定功能”]
...
```
损失= prediction_error + alpha * sum（|w_i|）
```
...
### sapgnry8fbpze8f666nc

b8xmf27mtmajou9jd0i7
...
X4 = x1 + 0.1 * rng.rand （n_samples）
* rng.randn（n_samples）

l2bgqxv7k8ulyu5wdo9f

c1ifk86dozqkyov0g0nn
...
7bf8gi2pouo212t11f44

lvamdux1fg64ubwkaojv


Feature_names = (
[f"info_{i}" for i in range(5)]
        + [f" cor_ {i}" for i in range(5)]
...
### ooyxar7axgrehqiu3fgf

sfd5ldgypqplnnfowsgk
...
defmutual_information (X, y, n_bins=10)：
n_samples, n_features = X.shape
Mi_scores = np.zero （n_features）

Y_vals, y_counts = np。独特的(y, return_counts = True)
P_y = y_counts / n_samples

对于range（n_features）中的f：
x_binned = discretise （X[:, f], n_bins）
X_vals, x_counts = np。独特的(x_binned return_counts = True)
P_x = dict（zip(x_vals, x_counts / n_samples)）

Mi = 0.0
对于x_vals中的xv：
对于yi， yv在enumerate（y_vals）中：
...
返回np。Sign (w) * np.maximum（np. max）Abs (w) - （0,0）


defl1_feature_selection (X, y, alpha=0.1, lr=0.01, epochs=500)：
n_samples, n_features = X.shape
W = np. 0 （n_features）
B = 0.0
...
代码文件在同一合成数据集上运行所有五个方法，并打印一个比较表，显示每个方法选择的特征。

## 使用它

使用scikit-learn，特征选择被内置到管道中：

”“python
从sklearn。Feature_selection import （）
VarianceThreshold,
mutual_info_classif,
RFE,
SelectFromModel,
）
从sklearn。导入Lasso， LogisticRegression
从sklearn。集成导入RandomForestClassifier

qlf6xs4eaoqmrwfg4jr3
