...
线性回归给出了这条直线。更重要的是，它引入了整个ML训练循环：def 模型，def 成本函数，优化参数。每个ML算法都遵循相同的模式。在这里用最简单的例子掌握它，你就会在任何地方认识它。
...
| Mean squared error | "Average of squared errors" | (1/n) * sum of (predicted - actual)^2, penalizing large errors  |
