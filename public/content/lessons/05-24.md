...
4. **基于跨的端到端（Lee et al., 2017）。**Transformer编码器。列举所有候选人的跨度，不超过长度上限。预测提及分数。预测每个跨度的先行概率。集群贪婪。现代违约。
5. **生成(2024 +)。**提示LLM：“列出本文中的每个代词及其先行词。”在简单的情况下工作得很好，在长文件和罕见的参考文献上挣扎。
...
两种故障模式值得注意。首先，LLM过度合并（“他”和“她”指的是两个不同的人）。其次，LLM们会悄悄地在冗长的文件中删除提及。始终使用跨度偏移检查进行验证。
...
1. 的方法。基于规则/基于神经跨度/LLM提示/混合型。一句话的原因。
2. 模型。命名检查点如果是神经的。
